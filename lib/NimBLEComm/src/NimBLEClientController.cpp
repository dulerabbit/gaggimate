...
bool NimBLEClientController::connectToServer() {
    ...
    if (!hasServerAddress) {
        ESP_LOGW(LOG_TAG, "No BLE server address available yet. Rescanning...");
        scan();
        return false;
    }
...
void NimBLEClientController::sendAdvancedOutputControl(bool valve, float boilerSetpoint, bool pressureTarget, float pressure,
                                                       float flow) {
    if (client != nullptr && client->isConnected() && outputControlChar != nullptr) {
...
void NimBLEClientController::onResult(NimBLEAdvertisedDevice *advertisedDevice) {
    ...
    const bool serviceMatch =
        advertisedDevice->haveServiceUUID() && advertisedDevice->isAdvertisingService(NimBLEUUID(SERVICE_UUID));
    const std::string deviceName = advertisedDevice->getName();
    const bool nameMatch = deviceName == "GPBLS" || deviceName.rfind("GPBL", 0) == 0 ||
                           deviceName.find("GaggiMate") != std::string::npos;

    if (serviceMatch || nameMatch) {
        ESP_LOGI(LOG_TAG, "Found target BLE device. Connecting...");
        scanner->stop();
        serverDevice = advertisedDevice;
        serverAddress = NimBLEAddress(advertisedDevice->getAddress());
