...
GitHubOTA::~GitHubOTA() {
    clearSemver(_version);
    clearSemver(_controller_version);
    clearSemver(_latest_version);
}

void GitHubOTA::clearSemver(semver_t &version) {
    semver_free(&version);
    version = {0, 0, 0, nullptr, nullptr};
}
...
void GitHubOTA::setControllerVersion(const String &controller_version) {
    String normalized = controller_version;
    clearSemver(_controller_version);
    if (normalized.isEmpty()) {
        _controller_version = from_string("");
        return;
    }
    ...
    _controller_version = from_string(normalized.c_str());
}
