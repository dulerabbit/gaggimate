...
void ShotHistoryPlugin::setup(Controller *c, PluginManager *pm) {
    ...
    pm->on("controller:manual:start", [this](Event const &) { startRecording(true); });
    pm->on("controller:manual:end", [this](Event const &) { endRecording(); });
...
void ShotHistoryPlugin::record() {
    ...
    if (shouldRecord && (controller->getMode() == MODE_BREW || controller->getMode() == MODE_MANUAL || extendedRecording)) {
        if (!isFileOpen) {
            if (!fs->exists("/h")) {
                fs->mkdir("/h");
            }
    ...
    if (!recording && !extendedRecording && isFileOpen) {
        ...
        if (duration <= 7500) { // Exclude failed shots and flushes
            ...
            if (manualShotInProgress) {
                Event event;
                event.id = "shot-history:manual:recorded";
                event.setString("id", currentId);
                event.setFloat("duration", static_cast<float>(header.durationMs) / 1000.0f);
                event.setFloat("volume", static_cast<float>(header.finalWeight) / WEIGHT_SCALE);
...
void ShotHistoryPlugin::startRecording(bool isManualShot) {
    Process *process = controller->getProcess();
    if (process != nullptr && process->getType() == MODE_BREW) {
    ...
    manualShotInProgress = isManualShot;
    sampleCount = 0;
    ioBufferPos = 0;
