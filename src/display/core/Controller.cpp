...
#include <display/core/process/ManualPressureProcess.h>
...
#else
    Serial.println(F("[LILYGO] setupPanel start"));
    driver = LilyGoDriver::getInstance();
    Serial.println(F("[LILYGO] driver instance ready"));
    driver->init();
    Serial.println(F("[LILYGO] driver init complete"));
#endif
}
#endif
...
void Controller::loop() {
    ...
    if (waitingForController && initialized && !clientController.isConnected() &&
        (lastReconnectAttempt == 0 || (now - lastReconnectAttempt) >= CONTROLLER_RECONNECT_INTERVAL_MS)) {
        clientController.scan();
        lastReconnectAttempt = now;
    }

    if (waitingForController && clientController.isConnected()) {
...
float Controller::getTargetTemp() const {
    ...
    case MODE_MANUAL:
        return settings.getTargetManualTemp();
    default:
        return 0;
    }
}
...
void Controller::setTargetTemp(float temperature) {
    ...
    case MODE_WATER:
        ...
        break;
    case MODE_MANUAL:
        settings.setTargetManualTemp(static_cast<int>(temperature));
        break;
    default:;
    }
    updateLastAction();
...
void Controller::raiseManualPressure() {
    adjustManualPressure(0.1f);
}

void Controller::lowerManualPressure() {
    adjustManualPressure(-0.1f);
}

void Controller::adjustManualPressure(float deltaBar) {
    float p = settings.getTargetManualPressure() + deltaBar;
    if (p > 9.0f) p = 9.0f;
    if (p < 0.0f) p = 0.0f;

    // Keep target stable at one decimal precision for display/control consistency.
    p = std::round(p * 10.0f) / 10.0f;

    settings.setTargetManualPressure(p);
    updateLastAction();
}
...
void Controller::updateControl() {
    ...
    if (active && systemInfo.capabilities.pressure) {
        ...
        if (proc->getType() == MODE_MANUAL) {
            targetPressure = settings.getTargetManualPressure();
            targetFlow = 0.0f;
            if (targetPressure <= 0.0f) {
                // Soak mode: relay stays active, but pump output is disabled until pressure target is raised again.
                clientController.sendOutputControl(true, 0.0f, targetTemp);
            } else {
                clientController.sendAdvancedOutputControl(true, targetTemp, true, targetPressure, targetFlow);
            }
            return;
        }
    }
...
void Controller::activate() {
    ...
    case MODE_WATER:
        ...
        break;
    case MODE_MANUAL:
        startProcess(new ManualPressureProcess());
        break;
    default:;
    }
    if (currentProcess != nullptr && currentProcess->getType() == MODE_BREW) {
        pluginManager->trigger("controller:brew:start");
    } else if (currentProcess != nullptr && currentProcess->getType() == MODE_MANUAL) {
        pluginManager->trigger("controller:manual:start");
    }
}
...
void Controller::deactivate() {
    ...
    } else if (lastProcess->getType() == MODE_MANUAL) {
        pluginManager->trigger("controller:manual:end");
    } else if (lastProcess->getType() == MODE_GRIND) {
