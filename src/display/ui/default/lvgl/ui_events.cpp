...
void onBrewTempLower(lv_event_t *e) {
    controller.getUI()->focusBrewSettingsTemp();
...
void onBrewTimeLower(lv_event_t *e) {
    controller.getUI()->focusBrewSettingsTarget();
    controller.getUI()->markProfileDirty();
...
void onManualScreen(lv_event_t *e) {
    controller.deactivate();
    controller.setMode(MODE_MANUAL);
    controller.getUI()->changeScreen(&ui_ManualBrewScreen, &ui_ManualBrewScreen_screen_init);
}

void onManualProcessToggle(lv_event_t *e) {
    controller.isActive() ? controller.deactivate() : controller.activate();
}

void onManualTempRaise(lv_event_t *e) {
    controller.raiseTemp();
    controller.getUI()->markDirty();
}

void onManualTempLower(lv_event_t *e) {
    controller.lowerTemp();
    controller.getUI()->markDirty();
}

void onManualPressureAdjust(lv_event_t *e, float deltaBar) { controller.getUI()->onManualPressureTouch(deltaBar); }
...
void onMenuScreenLoad(lv_event_t *e) {
    ...
    lv_obj_set_ext_click_area(ui_MenuScreen_btnManual, 15);
    lv_obj_set_ext_click_area(ui_MenuScreen_standbyButton, 20);
}

void onBrewScreenLoad(lv_event_t *e) {
    lv_obj_set_ext_click_area(ui_BrewScreen_startButton, 35);
    lv_obj_set_ext_click_area(ui_BrewScreen_profileSelectBtn, 25);
    lv_obj_set_ext_click_area(ui_BrewScreen_settingsButton, 35);
...
void onManualScreenLoad(lv_event_t *e) {
    lv_obj_set_ext_click_area(ui_ManualBrewScreen_downTempBtn, 30);
    lv_obj_set_ext_click_area(ui_ManualBrewScreen_upTempBtn, 30);
    lv_obj_set_ext_click_area(ui_ManualBrewScreen_goBtn, 25);
    lv_obj_set_ext_click_area(ui_ManualBrewScreen_backBtn, 20);
}
