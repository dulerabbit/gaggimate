...
## Display-Only Offline Test Mode

For fast UI iteration without connecting the machine controller, use the dedicated offline environment for your display:

```
pio run -e display_offline -t upload
```

For the Elecrow rotary display, use:

```
pio run -e elecrow_rotary_21_offline -t upload
```

Offline mode keeps the display interactive on USB power and bypasses controller BLE waiting/connect state in the UI.
Use the regular `display` or `elecrow_rotary_21` environment when validating real machine behavior.
